...
object EqualizerHelper {
     ...
     */
    fun init(audioSessionId: Int, context: Context): Boolean {
    ...
    fun saveSettings(context: Context, syncToApi: Boolean = true) {
        ...
        editor.putBoolean(Keys.PREF_EQ_ENABLED, isEnabled)
        editor.putString(Keys.PREF_EQ_PRESET, currentPreset.name)

        // Band-Levels in dB speichern
        for (band in 0 until numberOfBands) {
            val levelDb = currentBandLevels[band] / 100
            editor.putInt("${Keys.PREF_EQ_BAND_PREFIX}$band", levelDb)
        }

        editor.apply()
        Log.d(TAG, "Settings saved: enabled=$isEnabled, preset=${currentPreset.name}")
